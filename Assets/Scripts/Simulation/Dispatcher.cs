...
namespace Simulation
{
    public class Dispatcher : MonoBehaviour
    {
        ...
        [SerializeField] private Toggle toggleMaterialMap;
        ...
        private void OnRenderImage(RenderTexture src, RenderTexture dest)
        {
            ...
            if (_updateInterval % updateInterval == 0)
            {
                ...
                if (_cellPositionsToAdd.Count > 0)
                {
                    // initialize buffer
                    var toAddBuffer =
                        new ComputeBuffer(_cellPositionsToAdd.Count, sizeof(uint) * 2 + sizeof(float) * 8);
            ...
            // render final result
            Graphics.Blit(toggleMaterialMap.isOn ? resultMaterial : resultTexture, dest);
        }
        ...
        public void AddCell(Vector3 screenPosition, Chemical chemical, int radius)
        {
            ...
            var chemicalMaterial = chemical.Material;
            chemicalMaterial.RandWeight = Random.Range(0f, 1f);
        ...
        private static IEnumerable<uint2> CirclePositions(int2 center, int radius)
        {
            ...
            for (var r = 0; r < radius; r++)
            {
                for (var x = center.x - radius; x <= center.x + radius; x++)
                {
                    ...
                    {
                        if (y < 0) continue;
                        if (Mathf.Sqrt((x - center.x) * (x - center.x) + (y - center.y) * (y - center.y)) <= r + 1 &&
                            Mathf.Sqrt((x - center.x) * (x - center.x) + (y - center.y) * (y - center.y)) > r)
                        {
        ...
        {
            var t = new RenderTexture(Screen.width / cellWidth, Screen.height / cellHeight, 24,
